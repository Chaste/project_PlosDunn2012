...
#ifndef CROSSSECTIONMODELARCHIVER_HPP_
#define CROSSSECTIONMODELARCHIVER_HPP_
...
/**
 * CrossSectionModelArchiver handles the checkpointing (saving and loading)
 * of all CrossSectionModel objects. It has no explicit constructor (just 
 * uses a default one) and no member variables.
 */
template<unsigned DIM, class SIM>
class CrossSectionModelArchiver
{
public:
...
template<unsigned DIM, class SIM>
SIM* CrossSectionModelArchiver<DIM, SIM>::Load(const std::string& rArchiveDirectory, const double& rTimeStamp)
{
    /**
...
template<unsigned DIM, class SIM>
void CrossSectionModelArchiver<DIM, SIM>::Save(SIM* pSim)
{
...
#endif /*CROSSSECTIONMODELARCHIVER_HPP_*/
